...
test("schema 9 Writer generic v1 winner upgrades and reopens without manufacturing an Artifact", async () => {
  ...
  try {
    ...
    const upgraded = new DatabaseSync(value.temporary.path);
    for (const table of ["artifacts", "approval_challenges", "approvals", "response_claims", "publication_freshness"]) assert.equal(upgraded.prepare(`SELECT count(*) AS count FROM ${table}`).get()?.["count"], 0);
