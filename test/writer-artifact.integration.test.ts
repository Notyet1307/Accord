...
test("schema 9 Writer generic v1 winner upgrades and reopens without manufacturing an Artifact", async () => {
  ...
  try {
    ...
    authority = openAuthorityDatabase(value.temporary.path); authority.close(); authority = openAuthorityDatabase(value.temporary.path); const upgraded = new DatabaseSync(value.temporary.path); assert.equal((upgraded.prepare("PRAGMA user_version").get() as Row)["user_version"], 11); assert.equal((upgraded.prepare("SELECT count(*) AS count FROM artifacts").get() as Row)["count"], 0);
