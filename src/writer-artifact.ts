...
export function createWriterArtifactContract(database: DatabaseSync, context: PreparedProfileInvocation, h1: ReviewerDispositionHandoff): InvocationBoundOutputContract {
  ...
  return Object.freeze({ invocationId: context.invocationId, contextDigest: context.contextDigest, profile: "WRITER", profileVersion: context.profileVersion, outputSchema: context.outputSchema, materialize(currentContext: Readonly<PreparedProfileInvocation>, output: unknown) {
    ...
    const carrier: WriterArtifactCarrier = Object.freeze({ artifactId, artifactRevision: 1, contentMarkdown, contentDigest, materialAssertions: manifest, manifestDigest, artifactDigest, reviewerHandoffId: h1.handoffId, reviewerResultId: h1.resultId });
...
function parseCarrier(value: unknown, context: PreparedProfileInvocation): WriterArtifactCarrier {
  const raw = record(value, "Writer Artifact materialization"); exact(raw, ["artifactDigest", "artifactId", "artifactRevision", "contentDigest", "contentMarkdown", "manifestDigest", "materialAssertions", "reviewerHandoffId", "reviewerResultId"], "Writer Artifact materialization");
  ...
  return Object.freeze({ artifactId, artifactRevision: 1, contentMarkdown, contentDigest, materialAssertions, manifestDigest, artifactDigest, reviewerHandoffId, reviewerResultId });
}

function assertArtifactEligibility(database: DatabaseSync, context: PreparedProfileInvocation, artifact: WriterArtifactCarrier): void {
  const reviewer = database.prepare(`SELECT result.result_id FROM runtime_results result
    JOIN runtime_result_arrivals arrival ON arrival.result_id = result.result_id AND arrival.outcome = 'WINNER'
    ...
    JOIN runtime_result_entries link ON link.result_id = result.result_id
    JOIN board_entries entry ON entry.board_entry_id = link.board_entry_id AND entry.case_id = ?
    WHERE result.result_id = ? GROUP BY result.result_id
    HAVING count(*) = 2 AND sum(entry.entry_type = 'Critique') = 1 AND sum(entry.entry_type = 'VerificationResult') = 1`).get(context.caseId, artifact.reviewerResultId);
  if (reviewer === undefined) throw new TypeError("Writer Artifact does not bind an exact durable Reviewer winner");
...
export function parseWriterArtifactAuthority(database: DatabaseSync, context: PreparedProfileInvocation, materialization: DurableGenericMaterialization): WriterArtifactCarrier {
  if (context.profile !== "WRITER" || materialization.profile !== "WRITER" || materialization.writerArtifact === undefined || materialization.boardEntries.length !== 1 || materialization.boardEntries[0]?.entryType !== "ArtifactRef" || materialization.handoff === undefined) throw new TypeError("Writer winner must contain exactly one Artifact, ArtifactRef, and H2");
  const artifact = parseCarrier(materialization.writerArtifact, context); assertArtifactEligibility(database, context, artifact); const ref = materialization.boardEntries[0]; const expectedPayload = { artifactId: artifact.artifactId, artifactRevision: 1, artifactDigest: artifact.artifactDigest, contentDigest: artifact.contentDigest, manifestDigest: artifact.manifestDigest, materialAssertions: artifact.materialAssertions, reviewerHandoffId: artifact.reviewerHandoffId };
...
export function persistWriterArtifact(database: DatabaseSync, context: PreparedProfileInvocation, resultId: ResultId, materialization: DurableGenericMaterialization, createdAt: string): void {
  const artifact = parseWriterArtifactAuthority(database, context, materialization);
  database.prepare(`INSERT INTO artifacts (artifact_id, artifact_revision, schema_version, case_id, workflow_run_id, board_id, source_invocation_id, source_result_id, reviewer_result_id, reviewer_handoff_id, content_markdown, content_digest, material_assertions_json, manifest_digest, artifact_digest, created_board_revision, created_at)
    VALUES (?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(artifact.artifactId, ARTIFACT_SCHEMA_VERSION, context.caseId, context.workflowRunId, context.boardId, context.invocationId, resultId, artifact.reviewerResultId, artifact.reviewerHandoffId, artifact.contentMarkdown, artifact.contentDigest, json(artifact.materialAssertions), artifact.manifestDigest, artifact.artifactDigest, materialization.batchRevision, createdAt);
}
