...
export function createWriterArtifactContract(database: DatabaseSync, context: PreparedProfileInvocation, h1: ReviewerDispositionHandoff): InvocationBoundOutputContract {
  ...
  return Object.freeze({ ...(context.profileVersion === "accord.writer/v2" ? { providerInput: Object.freeze({ kind: "WRITER_ELIGIBLE_BASES", reviewerHandoff: h1, bases: Object.freeze([...evidence].map(([entryId, basis]) => Object.freeze({ entryId, contentDigest: basis.contentDigest, statement: basis.content })).concat(verifiedBasis === undefined ? [] : [verifiedBasis])) }) } : {}), invocationId: context.invocationId, contextDigest: context.contextDigest, profile: "WRITER", profileVersion: context.profileVersion, outputSchema: context.outputSchema, materialize(currentContext: Readonly<PreparedProfileInvocation>, output: unknown) {
