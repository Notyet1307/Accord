...
import { REVIEWER_OUTPUT_SCHEMA, REVIEWER_PROFILE_VERSION, REVIEWER_V2_PROFILE_VERSION } from "./profile-context.js";
const REVIEWER_PROFILE_VERSIONS: Record<string, true> = { [REVIEWER_PROFILE_VERSION]: true, [REVIEWER_V2_PROFILE_VERSION]: true };
import {
...
function normalizeView(decisionValue: ProfileContextDecision): ReviewerContextView {
  ...
  if (view["schemaVersion"] !== PROFILE_CONTEXT_VIEW_VERSION || view["kind"] !== "REVIEWER_CONTEXT" || view["profile"] !== "REVIEWER" || REVIEWER_PROFILE_VERSIONS[String(view["profileVersion"])] !== true || view["outputSchema"] !== REVIEWER_OUTPUT_SCHEMA || !Array.isArray(view["entries"])) throw new TypeError("C03 decision is not the fixed Reviewer Context");
...
export function parseReviewerDispositionHandoff(value: DurableGenericMaterialization): ReviewerDispositionHandoff {
  ...
  if (materialization["schemaVersion"] !== GENERIC_MATERIALIZATION_SCHEMA_VERSION || materialization["profile"] !== "REVIEWER" || REVIEWER_PROFILE_VERSIONS[String(materialization["profileVersion"])] !== true || materialization["outputSchema"] !== REVIEWER_OUTPUT_SCHEMA || !Array.isArray(materialization["boardEntries"]) || materialization["boardEntries"].length !== 2) throw new TypeError("Reviewer materialization envelope is invalid");
