...
function parseChoiceFreeText(value: unknown, textContract: "r003" | "r004" | "r005"): string {
  ...
  if (textContract === "r005") {
    const content = body["content"];
    if (!content.isWellFormed() || Buffer.byteLength(content) > 1_048_576) throw new TypeError("R005 text exceeds the bounded frame or is invalid");
    return content; // R005 owns semantic limits and durably replies without rewriting question bytes.
  }
