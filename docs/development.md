...
**R005 已有独立 MagicChat 私聊及真实 Adapter 装配，实际部署须读取当前执行证据。** [R005CqaConsumer](../src/driver/r005-cqa.ts) 仍独占 Case、Operation 和 SQLite：`configureChat` 冻结无秘密的授权/传输配置，`receive` 持久接收官方消息，`flush` 发送并等待相关确认；原 `accept / advance / collect / snapshot` API 保持。字段不全不调用模型，后续问题保留明确范围；`/status`、`/stop`、`/new`、`/export` 为确定性命令。正式接受只来自绑定完整预览和精确 Artifact 的官方 choice，不接受自由文本“同意”。

真实入口为 `node scripts/run-r005.mjs --live --database <绝对路径> --config <绝对路径> --credentials <绝对路径>`。配置为 `{schemaVersion:"accord.r005-live/v1", binding:CqaBinding, chat:CqaChatConfiguration}`；0600 凭据文件只含两个互异 reference 对应的值。启动前检查规范私有路径、授权到期、不可变 Binding、文件/目录碰撞和秘密反射；不读取环境凭据、不自动重连、重试 Start 或回退替身。授权到期、信号或 socket 错误停止本地驱动，保留未知远端事实；不凭本地退出宣称外部已停止。[真实驱动的受控协议场景](../test/r005-live-driver.integration.test.ts)不是实际 IM 或模型运行。

[CqaRunServiceAdapter](../src/transports/cqa-run-service.ts) 仍接收显式 `(binding, credential)`，不读取凭据文件或签发 Grant；它消费操作员另行取得的真实部署/逐 Run 观测，不拥有 Docker socket。managed 与 offline Grant 不互换。未知 `message.send` 不自动重发或按正文猜测；缺失相关确认会阻止冲突的新回复与查询，需核对原发送而非删除数据库。

数据库父目录须预先存在、规范绝对路径且权限 0700；SQL 表形状仍为 `r005_cqa` schema 1，r5 状态快照为 version 3。只接受新库或相同 Binding 的 version 3；version 1/2 在写连接前拒绝，不迁移历史库。回归要求拒绝后主库字节不变、WAL 无 frame；辅助文件存在不表示发生迁移。输入根目录的父目录须存在，模块创建私有逐操作目录和 0400 输入，不覆盖冲突或清理未知操作。准备器依赖 POSIX hard link/fsync，信任同 UID 与 root；不是抵抗同 UID 恶意进程的沙箱。

Binding 的规范化复用既有模式：递归排序对象键，再由 `JSON.stringify` 序列化。未设置的可选字段与 own-`undefined` 字段等价，后者不写入 JSON；不另加拒绝规则。回归验证重启后省略/显式 `undefined` 的消息重放仍是同一 Operation；实际冒烟验证先前提交创建的有效数据库可重开且 Binding 摘要不变。不自动修复或删除此前已有的非法 JSON 快照。
...
**资格执行分层，资格状态来自实际执行记录。** Node 24.19.0 Permission Model 不支持所需同步 fsync/祖先元数据与符号链接场景。按[唯一分层合同](specs/r005-compliance-query-consumption.md#本地资格验证的分层边界)，r4 三个及 r5 两个 R005 文件系统入口使用既有 operator-owned OS 无网/文件隔离和 runtime guard；其他入口保留 Node Permission Model。不授予全局文件权限，不放宽生产输入校验或原生网络静态例外。完整资格须绑定干净候选提交、真实 launcher/profile、拒绝探针和精确工具链；普通 CI 不能替代。历史最终 r4 资格由 [PR #80](https://github.com/Notyet1307/Accord/pull/80) 索引，不重新绑定 r5；新证据沿 [当前入口](work/current.md) 查找。
...
R003 schema 12、R004 独立 schema 1 与 R005 `r005_cqa` schema 1 / snapshot version 3 不互换数据库；R005 version 1/2 也不自动升级。保留未知操作、待确认消息及证据；代码回退不能当作数据回滚，删除数据库也不是恢复。
