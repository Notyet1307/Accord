...
The explicitly authorized [R005 layered validation](../specs/r005-compliance-query-consumption.md#本地资格验证的分层边界) retains that outer OS boundary. Only its three filesystem test entrypoints omit the incompatible Node Permission Model; they still use a cleared environment and the unchanged runtime capability guard. Build/static checks, pure CQA wire tests, R003/R004, and denied-capability regressions retain their existing Node restrictions. No global Node filesystem grant or generic unrestricted runner is added; the outer OS boundary and actual operator execution evidence remain mandatory.
