...
当前 Accepted 增量为 [R005 合规查询对话试点](docs/product/releases/r005-compliance-query-conversational-pilot.md)，唯一行为合同见 [R005-CQA/r8](docs/specs/r005-compliance-query-consumption.md)。私聊软件、实际部署与人工验收分别取证，当前修复任务为 [Issue #87](https://github.com/Notyet1307/Accord/issues/87)，执行入口与实际回执见 [current](docs/work/current.md)。R004 的 SAS 对话仍为独立离线实现；R003 的联调记录不证明其他 Release 已可使用。开发授权与交付规则见 [delivery gate](docs/agents/delivery-gate.md)，GitHub 拥有任务状态。
